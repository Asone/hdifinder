...
use bitcoin::{Address, network::constants::Network, util::bip32::{ExtendedPrivKey, DerivationPath, ExtendedPubKey}};
...
fn address_compute(pubkey: ExtendedPubKey) -> [(&'static str,String); 3] {
    ...
    let p2wpkh: String = Address::p2wpkh(&pubkey.public_key, Network::Bitcoin).unwrap().to_string();
    
    let p2shwpkh: String = Address::p2shwpkh(&pubkey.public_key, Network::Bitcoin).unwrap().to_string();
    
    [("p2pkh",p2pkh),("p2wpkh",p2wpkh),("p2shwpkh",p2shwpkh)]
}

fn executor(address: &str, wallet_conf: &WalletConf, execution_config: ExecutionConf) -> Option<(usize, String, String)> {
                    let start = execution_config.start;
                    let end = execution_config.end;

                    '_outer: for i in start..end{
                        let hd_path = wallet_conf.account.address_at(0, i.try_into().unwrap()).unwrap();
                        let private_key = self::get_private_key(wallet_conf.seed,&hd_path);
...
fn main() { 
    ...
    match self::get_mnemonic(mnemonic){
        Ok(mnemonic) => {
            ...
            let wallet_conf: WalletConf = WalletConf {
                            seed, 
            ...
            (0..slices).into_par_iter().for_each(|slice|{
                ...
                match self::executor(&config.address, &wallet_conf, execution_config){
                    Some(result) => {
                        println!("address {} found at index {}. address type: {}",result.1, result.0 ,result.2);
